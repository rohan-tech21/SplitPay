...
export const TxStatusModal: React.FC<TxStatusModalProps> = ({ progress, onClose }) => {
  if (!progress.isProcessing && progress.step !== "Success" && progress.step !== "Failed") {
    return null;
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md fade-in">
      <div className="premium-card rounded-2xl max-w-md w-full p-6 space-y-6 border border-[rgba(184,115,51,0.3)] shadow-2xl text-center">
        {/* Status Indicator */}
